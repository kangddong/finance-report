...
- **종목 관리**: `종목.md` 파일에 정의된 보유종목과 관심종목을 기반으로 분석 우선순위를 정합니다.
- **보고서 생성**: 반드시 전용 **스킬**을 사용하여 데이터의 일관성을 유지합니다.
    - `report`: 종목 정보 분석 및 투자 조언 리포트 생성.
    - `toss-capture`: 토스증권 차트 캡처 및 이미지 저장.
    - `sync-dashboard`: 생성된 JSON 데이터를 `data.js`에 동기화.
    - `deep-dive`: 전문가급 심층 분석 HTML 생성 및 연동.
- **파일 경로**:
    - 리포트: `report/YYYY-MM-DD.json`
    - 이미지: `report/images/YYYY-MM-DD/[종목명].png`

### 2. 시스템 아키텍처 및 코드 작성 (`ARCHITECTURE.md` 준수)
- **모듈화**: 모든 UI 수정은 `dashboard/css/`와 `dashboard/js/sections/` 내의 개별 모듈에서 수행합니다.
- **데이터 접근**: UI 모듈은 `data.js`를 직접 참조하지 않고, 반드시 **`js/db.js`**의 통합 인터페이스를 사용합니다.
- **유틸리티 활용**: 마크다운 파싱이 필요한 경우 `js/utils/markdown.js`를 재사용합니다.

### 3. 문서화 및 검증
- **문서 동기화**: 아키텍처나 데이터 구조에 변경이 생길 경우 `docs/` 디렉토리의 문서를 즉시 업데이트합니다.
...
### 대시보드 기능 확장
1. `docs/DEVELOPMENT.md` 가이드에 따라 HTML/CSS/JS 모듈 분리 작업 수행.
