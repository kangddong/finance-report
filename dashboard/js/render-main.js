document.addEventListener('DOMContentLoaded', async () => {
    try {
        const { data, wordsData } = await loadDashboardData();
        initDashboard(data, wordsData);
    } catch (error) {
        console.error(error);
        const errorMsg = '보고서 데이터를 불러오는데 실패했습니다. (CORS 보안 설정 또는 파일 누락)';
...
function initDashboard(dataContext, financeWords) {
    const history = dataContext?.REPORTS_HISTORY || [];
    ...
    if (history.length > 0) {
        ...
        // 헬퍼 함수를 통해 가장 최신 리포트 결정
        const latestReport = getLatestReport(history);
        const latestIndex = history.findIndex(r => r.date === latestReport.date);
        selector.value = latestIndex;
        
        renderDashboard(history[latestIndex], dataContext);

        // 날짜 선택 이벤트 리스너
        selector.addEventListener('change', (e) => {
            const selectedIndex = e.target.value;
            renderDashboard(history[selectedIndex], dataContext);
    ...
    shockTabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tab === 'timeline') {
                ...
                renderPolicyPage(dataContext);
            }
        });
    });

    // --- Company & Sector Sub-tab Logic ---
...
function renderDashboard(data, dataContext) {
    ...
    renderCompanyAnalysisGrid(dataContext);
    renderDeepDiveGrid(dataContext);
    renderSectorAnalysis(dataContext);
    renderExternalFactors(dataContext);
...
function renderCompanyAnalysisGrid(dataContext) {
    const grid = document.getElementById('company-analysis-grid');
    if (!grid) return;

    const analyses = dataContext?.COMPANY_ANALYSIS_ITEMS || [];
    grid.innerHTML = '';
...
function renderDeepDiveGrid(dataContext) {
    const grid = document.getElementById('deep-dive-grid');
    const items = dataContext?.DEEP_DIVE_ITEMS || [];
...
function renderExternalFactors(dataContext) {
    ...
    const events = dataContext?.EXTERNAL_SHOCK_EVENTS || [];
    summary.innerHTML = '';
    timeline.innerHTML = '';
...
function renderSectorAnalysis(dataContext) {
    const summary = document.getElementById('sector-analysis-summary');
    const list = document.getElementById('sector-analysis-list');
    if (!summary || !list) return;

    const sectors = dataContext?.SECTOR_ANALYSIS_ITEMS || [];
    const companyAnalyses = dataContext?.COMPANY_ANALYSIS_ITEMS || [];
    summary.innerHTML = '';
    list.innerHTML = '';
...
function renderPolicyPage(dataContext) {
    const container = document.getElementById('policy-content');
    const policyData = dataContext?.EXTERNAL_POLICY_DATA || {};
    if (!container || Object.keys(policyData).length === 0) return;

    container.innerHTML = '';
    const grid = document.createElement('div');
    grid.id = 'policy-grid';
    grid.style.cssText = 'display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; margin-top: 1rem;';

    Object.entries(policyData).forEach(([key, pm]) => {
        const card = document.createElement('div');
        card.className = 'policy-pillar-card clickable-shock';
        card.style.cssText = 'cursor: pointer; transition: transform 0.2s; border: 1px solid rgba(129, 140, 248, 0.3);';
        card.onclick = () => showPolicyDetail(key, dataContext);
    ...
    detailsContainer.id = 'policy-detail-view';
    detailsContainer.style.display = 'none';
    container.appendChild(detailsContainer);
}

function showPolicyDetail(key, dataContext) {
    const policyData = dataContext?.EXTERNAL_POLICY_DATA || {};
    const pm = policyData[key];
    if (!pm) return;
    ...
    const backBtn = detailView.querySelector('.policy-back-btn');
    if (backBtn) {
        backBtn.addEventListener('click', hidePolicyDetail);
    }
}
